...
| GATE   | 항목                                     | 실패 시                     |
| ------ | ---------------------------------------- | --------------------------- |
| GATE-1 | error-registry 금지 패턴 스캔            | 즉시 반환 + 수정 지시       |
...
## PHASE 4: 자기 수정 루프 (Self-Correction)
...
3. 결과:
   ├─ 모두 통과 → Reviewer에게 전달
   ├─ 일부 실패 → 에러 + 수정 지시 → Worker 반환 (Loop)
   └─ 심각 실패 → CEO 직접 재작업 지시 + error-registry 기록
4. Reviewer 승인 → CEO 최종 확인
5. CEO 자가점검 (PHASE 0-3 재실행)
6. 사용자 전달
```

**최대 루프: 3회**
3회 초과 시 → CEO가 사용자에게 보고 + 근본 원인 분석 + 대안 제시
